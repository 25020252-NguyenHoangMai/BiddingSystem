...
ALTER TABLE [dbo].[Item] CHECK CONSTRAINT [FK_Item_Users]
    GO
ALTER TABLE [dbo].[AuctionSession]  WITH CHECK ADD  CONSTRAINT [CK_AuctionSession_Status] CHECK  (([status]='CANCELLED' OR [status]='FINISHED' OR [status]='RUNNING' OR [status]='OPEN' OR [status]='PAID'))
    GO
