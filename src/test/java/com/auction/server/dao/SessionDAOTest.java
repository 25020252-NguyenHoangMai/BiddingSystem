...
class SessionDAOTest {
    ...
    @Test
    void testInsertSession_ThrowsSQLException() throws SQLException {
        AuctionSession session = new AuctionSession("ss1", mock(Item.class), LocalDateTime.now(), LocalDateTime.now().plusDays(1));
        Item item = mock(Item.class);
    ...
    @Test
    void testGetSessionById_Found_Success() throws SQLException {
        when(connection.prepareStatement(anyString())).thenReturn(preparedStatement);
    ...
    @Test
    void testGetSessionById_ItemNotFound_ThrowsException() throws SQLException {
        when(connection.prepareStatement(anyString())).thenReturn(preparedStatement);
        when(preparedStatement.executeQuery()).thenReturn(resultSet);
        when(resultSet.next()).thenReturn(true);
        when(resultSet.getString("itemId")).thenReturn("item1");
    ...
    @Test
    void testExistsActiveSessionByItemId_True() throws SQLException {
        when(connection.prepareStatement(anyString())).thenReturn(preparedStatement);
        when(preparedStatement.executeQuery()).thenReturn(resultSet);
        when(resultSet.next()).thenReturn(true);
    ...
    @Test
    void testExistsActiveSessionByItemId_NullParam_ThrowsException() {
